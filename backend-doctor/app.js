...
async function init() {
    ...
    app.post('/release', (req, res) => releaseReportRoute(req, res));
...
async function releaseReportRoute(req, res) {
    try {
        await query.initialize();
        const result = await query.releaseReport(req.body.d_name, req.body.disease, 
            req.body.medicine);
        res.status(200).send(result);
    } catch (error) {
        console.error(`error on releaseReportRoute: ${error}`);
    }
}
